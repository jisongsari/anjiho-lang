...
function run(code, input) {
	...
	if (statements[0] !== '진짜' || !statements.slice(-1)[0].startsWith('입니다!')) {
		runElem.disabled = false;
		speedElem.disabled = false;
		outputElem.style.borderColor = 'red';
		return outputElem.value = '지랄똥싸네';
	}
	...
	function execute(statement) {
		if (statement.includes('어푸') && statement.includes('!')) { // IF GOTO
		...
		if (statement.includes('안')) {
			const variablePointer = statement.split('안')[0].split('아').length;
			const setteeValue = evaluate(statement.split('안')[1]);
			variables[variablePointer] = setteeValue;
		}

		if (statement.includes('지') && statement[statement.length - 1] === '?') {
			printOut(String(evaluate(statement.slice(1, -1))));
			return;
		}

		if (statement.includes('지') && statement[statement.length - 1] === '호') {
			if (statement === '지호') printOut('\n');
			printOut(stringify(evaluate(statement.slice(1, -1))));
			return;
		}

		if (statement.includes('똥')) {
			pointer = evaluate(statement.split('똥')[1]) - 1;
		}

		if (statement.indexOf('응가') === 0) {
			return evaluate(statement.split('응가')[1]);
		}
	}

	function parse() {
		if (statements[pointer].startsWith('입니다!')) stop();
		if (STOP) {
			STOP = false;
			stop();
		}
